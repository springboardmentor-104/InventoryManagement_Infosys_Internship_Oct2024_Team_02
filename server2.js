...
app.post('/verify-otp', async (req, res) => {
    const { otp } = req.body; // Only OTP should be posted
...
app.post('/reset-password', async (req, res) => {
    ...
    try {
        ...
        const resetToken =Math.floor(100000 + Math.random() * 900000).toString();// Re-using OTP as a reset token
        user.otp = resetToken;
        user.otpExpires = Date.now() + 3600000; // 1 hour expiration
        await user.save();
        console.log(`Generated OTP: ${resetToken}`); // Add this line before sending the OTP email
